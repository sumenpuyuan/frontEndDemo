...
$sendTime=date("Y-m-d")."+".date("h:i:sa");
// 错误报告
...
if(@$action == "postmsg"){
	//插入数据
	mysql_query("INSERT INTO messages (`user`,`msg`,`time`,`sendTime`) 
	             VALUES ('$name','$message',".time().",'$sendTime')",$dbconn);
	//删除数据(因为我们默认值存储10条数据)
...
//查询数据
$messages = mysql_query("SELECT user,msg,sendTime
						 FROM messages
						 WHERE time>$time
						 ",$dbconn);
//是否有新记录
...
if($status_code == 1){ //如果有记录
	while($message = mysql_fetch_array($messages)){
		...
		echo "\t\t<sendTime>$message[sendTime]</sendTime>\n";
